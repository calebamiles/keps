...
func NewRepo(routingInfo routing.Info, underlying git.Repo, submitChanges github.PullRequestCreator, token auth.TokenProvider) (Repo, error) {
	r := &repository{
...
func (r *repository) SubmitChanges() (string, error) {
	...
	// add
	err := r.underlying.Add(r.routingInfo.ChangesUnderPath())
	if err != nil {
		return "", err
	}
